...
import (
	...
	"github.com/yai/modules/testrunner/app/controllers"
	"github.com/yai/yai"
	"io"
	"io/ioutil"
	"net/http"
...
var cmdTest = &Command{
	...
	Long: `
Run all tests for the Yai app named by the given import path.

For example, to run the booking sample application's tests:

    yai test github.com/yai/samples/booking dev
...
For example, to run all of UserTest:
    ...
    yai test outspoken test UserTest

or one of UserTest's methods:

    yai test outspoken test UserTest.Test1
`,
}
...
func testApp(args []string) {
	...
	for _, module := range yai.Modules {
		if module.ImportPath == yai.Config.StringDefault("module.testrunner", "github.com/yai/modules/testrunner") {
...
You can add it to a run mode configuration with the following line:
	...
	module.testrunner = github.com/yai/modules/testrunner

`)
	}

	// Create a directory to hold the test result files.
	resultPath := path.Join(yai.BasePath, "test-results")
	...
	defer cmd.Kill()
	yai.INFO.Printf("Testing %s (%s) in %s mode\n", yai.AppName, yai.ImportPath, mode)
	...
	var (
		...
		resp       *http.Response
		baseUrl    = fmt.Sprintf("http://127.0.0.1:%d", yai.HttpPort)
	)
	for i := 0; ; i++ {
	...
	module, _ := yai.ModuleByName("testrunner")
	TemplateLoader := yai.NewTemplateLoader([]string{path.Join(module.Path, "app", "views")})
