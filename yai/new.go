...
var cmdNew = &Command{
	...
	Short:     "create a skeleton Yai application",
	Long: `
New creates a few files to get a new Yai application running quickly.
...
For example:
    ...
    yai new import/path/helloworld

    yai new import/path/helloworld import/path/skeleton
`,
}
...
func setApplicationPath(args []string) {
	...
	yaiPkg, err = build.Import(yai.YAI_IMPORT_PATH, "", build.FindOnly)
	if err != nil {
		errorf("Abort: Could not find Yai source code: %s\n", err)
	}
	...
	if basePath == "." {
		...
		// is a subdirectory such as $GOROOT/src/path/to/yaiapp
		basePath += "/"
	}
}
...
func setSkeletonPath(args []string) {
	...
	if len(args) == 2 { // user specified
		...
		// use the yai default
		skeletonPath = filepath.Join(yaiPkg.Dir, "skeleton")
	}
}
